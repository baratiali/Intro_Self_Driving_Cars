    ...
    if (matrixfile.is_open()) {
        while (getline (matrixfile, line)) {
            ...
            while(ss >> i) {
                ...
                if (ss.peek() == ',' || ss.peek() == ' ') {//In other words, the code finds a float number and appends the number to the vector called row. The line ss.peek() looks at the next character to see if it is a comma or a space and ignores commas or spaces.
                    ss.ignore();
                }
            }
